...
export default function Signin({ setSignin }: SigninProps) {
  ...
  const handleSignInWithGoogle = async () => {
    try {
      ...
      window.alert("Google singin failed");
    }
  };
  ...
  return (
    <div className="absolute z-50 bg-white w-2/3 rounded-lg p-2 shadow-lg shadow-blue-400 flex">
      <button onClick={() => setSignin(false)} className="absolute right-5">
        <IoMdCloseCircleOutline className="text-3xl text-blue-500" />
      </button>
      <div className="bg-slate-100 px-10 py-3 w-96 rounded-md">
        ...
        <button
          ...
          className="bg-white text-black border py-2 w-full rounded-lg mt-3 flex justify-center items-center text-sm hover:scale-105 duration-300"
        >
          <svg
